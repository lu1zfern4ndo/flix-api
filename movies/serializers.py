...
class MovieSerializer(serializers.ModelSerializer):
    ...
    def get_rate(self, obj):
        ...
        return round(rate, 1) if rate else 0

        '''
        # 1º Veirficar se há alguma avaliação, senão houver não faz sentindo
        # entrar nesse if
        if reviews:
            # No caso de haver alguma avaliação, então tá na hora de fazer a
            # contagem para saber quantas estrelas existe cada filme
        ...
        # Senão houver nenhuma avaliação, retorna direto um null de cara,
        # nem perde tempo verficando
        return None
        '''
